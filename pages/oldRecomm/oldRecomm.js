...
Page({
  ...
  onLoad: function (options) {
    this.setData({
      oldRecom:data.oldRecom
    })
    wx.getStorage({
      key: 'like',
      success:(res)=>{
        this.like_data=res.data
        for(let i=0;i<this.data.oldRecom.length;i++){
          for(let j=0;j<this.like_data.length;j++){
            if(this.like_data[j].id==this.data.oldRecom[i].id){
              this.setData({
                ["oldRecom["+i+"].Islike"]:true
              })
              break;
            }else{
              this.setData({
                ["oldRecom["+i+"].Islike"]:false
              })
            }
          }
        }
  ...
  like:function(e){
    ...
    var Islike="oldRecom["+index+"].Islike"
    var like=this.data.oldRecom[index].Islike
    this.setData({
      [Islike]:!like
    })
    if(this.data.oldRecom[index].Islike){
      var i=0;
  ...
  toDetail:function(e){
    var pv = JSON.stringify(this.data.oldRecom[e.currentTarget.dataset.index].pv);
    wx.navigateTo({
